...
public class CalendarView extends ImageView {
	...
	private void initCells() {
		class _calendar {
			public int day;
			public int month; // -1:previous 1:next 0:current

			public _calendar(int d, int m) {
				day = d;
				month = m;
			}
		}
		...
		for (int i = 0; i < tmp.length; i++) {
			...
			for (int d = 0; d < n.length; d++) {
				...
				else if (i == 0) { // previous month
					tmp[i][d] = new _calendar(n[d], PREVIOUS_MOUNT);
				} else { // next month
		...
		for (int week = 0; week < mCells.length; week++) {
			for (int day = 0; day < mCells[week].length; day++) {
				if (tmp[week][day].month == CURRENT_MOUNT) { // current month
					if (tmp[week][day].day == thisDay
							&& tmp[week][day].month == 0) { // today is Blue
						mToday = mCells[week][day] = new BlueCell(
						...
						mCells[week][day] = new Cell(tmp[week][day].day,
								new Rect(Bound), CELL_TEXT_SIZE);
				} else if (tmp[week][day].month == PREVIOUS_MOUNT) { // previous
																		// month
																		// is
																		// Gray
					mCells[week][day] = new GrayCell(tmp[week][day].day,
							new Rect(Bound), CELL_TEXT_SIZE);
				} else { // next month LTGray
				...
				// get today
				if (tmp[week][day].day == thisDay && tmp[week][day].month == 0) {
